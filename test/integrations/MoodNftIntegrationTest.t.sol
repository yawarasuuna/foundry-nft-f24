// SPDX-License-Identifier: SEE LICENSE IN LICENSE
pragma solidity ^0.8.18;

import {console2, Test} from "forge-std/Test.sol";
import {DeployMoodNft} from "../../script/DeployMoodNft.s.sol";
import {MoodNft} from "../../src/MoodNft.sol";

contract MoodNftIntegrationTest is Test {
    MoodNft private moodNft;
    DeployMoodNft private deployerMoodNft;

    string public constant HAPPY_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string public constant SAD_SVG_IMAGE_URI =
        "data:image/svg+xml;base64,";
    string public constant HAPPY_SVG_URI =
        "data:application/json;base64,";
    string public constant SAD_SVG_URI =
        "data:application/json;base64,";

    address MINTER = makeAddr("minter");
    address MINTER2 = makeAddr("minter2");

    function setUp() public {
        deployerMoodNft = new DeployMoodNft();
        moodNft = deployerMoodNft.run();
    }

    function testNamesIsCorrect() public view {
        string memory expectedName = "Mood";
        string memory actualName = moodNft.name();

        assertEq(keccak256(abi.encodePacked(expectedName)), keccak256(abi.encodePacked(actualName)));
    }

    function testSymbolIsCorrect() public view {
        string memory expectedSymbol = "MO";
        string memory actualSymbol = moodNft.symbol();

        assertEq(keccak256(abi.encodePacked(expectedSymbol)), keccak256(abi.encodePacked(actualSymbol)));
    }

    modifier minterMints() {
        vm.prank(MINTER);
        moodNft.mintNft();
        _;
    }

    function testViewTokenURIIntegration() public minterMints {
        console2.log(moodNft.tokenURI(0));
    }

    /*//////////////////////////////////////////////////////////////
                               FLIP MOOD
    //////////////////////////////////////////////////////////////*/

    function testIsAuthorizedToFlipMood() public minterMints {
        vm.prank(MINTER2);

        vm.expectRevert(MoodNft.MoodNft__UnauthorizedAccessToFlipMood.selector);
        moodNft.flipMood(0);
    }

    modifier moodFlipper() {
        vm.prank(MINTER);
        moodNft.flipMood(0);
        _;
    }

    function testFlipTokenToSad() public minterMints moodFlipper {
        string memory actualSadTokenURI = moodNft.tokenURI(0);
        string memory expectedSadTokenURI = SAD_SVG_URI;

        console2.log(moodNft.tokenURI(0));
        console2.log(SAD_SVG_URI);

        assertEq(keccak256(abi.encodePacked(actualSadTokenURI)), keccak256(abi.encodePacked(expectedSadTokenURI)));
    }

    function testFlipTokenToBackToHappy() public minterMints moodFlipper moodFlipper {
        string memory actualTokenURI = moodNft.tokenURI(0);
        string memory expectedTokenURI = HAPPY_SVG_URI;

        console2.log(moodNft.tokenURI(0));
        console2.log(HAPPY_SVG_URI);

        assertEq(keccak256(abi.encodePacked(actualTokenURI)), keccak256(abi.encodePacked(expectedTokenURI)));
    }
}
